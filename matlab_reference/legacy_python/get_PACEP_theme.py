...
def get_PACEP_theme(UTM_X, UTM_Y, terrain, complexity, reg, include_peatlands, Npreq, NPL, Nlay, cert_fun_choice='FRAFA_apr2023'):
    # Load data based on region
    region_files = {
        'Jylland': 'Jylland_pacep_til_usikkerhed.mat',
        'Fyn': 'fyn_pacep_til_usikkerhed.mat',
        'Fyn-MST': 'fyn_pacep_til_usikkerhed_MST.mat',
        'Sjælland': 'Sjaelland_pacep_til_usikkerhed.mat'
    }
    ...
    if reg in ['Jylland', 'Sjælland']:
        ...
        info_table = in_data['fynpaceppos']
    ...
    coords = PACEP['InfoTable'][['xutm_euref89_utm32', 'yutm_euref89_utm32']].values
    _, idx = np.unique(coords, axis=0, return_index=True)
    logind_dupe = np.zeros(len(coords), dtype=bool)
    logind_dupe[idx] = True

    model_year = PACEP['Model_Year']
    geophys_year = PACEP['InfoTable']['m_year'].values
    ...
    for key in ['InfoTable', 'Model_Depths', 'Model_Year', 'DepthInterfaceAboveModel', 'ThickGeophysModel']:
        PACEP[key] = PACEP[key][logind]

    nx, ny = len(UTM_X), len(UTM_Y)
    maxl = PACEP['Model_Depths'].shape[1]
    Grid = np.zeros((ny, nx, maxl))

    cert_fun = certainty_function_definer(cert_fun_choice)

    xs = PACEP['InfoTable']['xutm_euref89_utm32'].values
    ys = PACEP['InfoTable']['yutm_euref89_utm32'].values
    DOIs = PACEP['InfoTable']['doilower'].values
    ...
    # SearchRadius = 1 cell (matches MATLAB get_PACEP_theme)
    ...
    # Fill out missing DOI with 15 (matches MATLAB get_PACEP_theme)
    local_doi[np.isnan(local_doi)] = 15
    ...
    var0map = (0.5 * np.maximum(5, 0.5 * local_depth)) ** 2
