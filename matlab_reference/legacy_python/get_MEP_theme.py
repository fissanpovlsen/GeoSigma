...
def get_MEP_theme(UTM_X, UTM_Y, terrain, complexity, reg, include_peatlands, Npreq, NPL, Nlay, cert_fun_choice='FRAFA_apr2023'):
    ...
    # Classify the MEP data subtype: 1 = wenner_2d, 0 = not wenner_2d
    type_strings = MEP['InfoTable']['datasubtype'].values
    MEP['Type'] = np.array([1 if str(t)[:3] == 'wen' else 0 for t in type_strings])
    ...
    for key in ['InfoTable', 'Model_Depths', 'Model_Year', 'DepthInterfaceAboveModel', 'ThickGeophysModel', 'Type']:
    ...
    SearchRadius = 2  # in units of cells (matches MATLAB get_MEP_theme)
    ...
    # Fill out missing DOI with the mean DOI (matches MATLAB get_MEP_theme)
    local_doi[np.isnan(local_doi)] = np.mean(DOIs[~np.isnan(DOIs)])
    ...
    # var0 depends on acquisition type: wenner_2d vs. everything else
