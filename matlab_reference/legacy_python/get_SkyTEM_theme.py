...
def get_SkyTEM_theme(UTM_X, UTM_Y, terrain, complexity, reg, include_peatlands, Npreq, NPL, Nlay, cert_fun_choice='FRAFA_apr2023'):
    ...
    if reg in ['Jylland', 'Sjælland']:
        ...
        info_table = in_data['fynskytempos']
    ...
    SearchRadius = 6  # in units of cells (matches MATLAB get_SkyTEM_theme)
    ...
    # Region-specific Palaeogene (Preq) depth/thickness used to estimate missing DOIs.
    # NOTE: these column offsets are translated 1:1 from the MATLAB. As with the
    # `Npreq - NPL` slicing elsewhere in the theme files, the 1-based -> 0-based
    # index convention still needs to be confirmed during MATLAB-parity validation.
    if reg == 'Jylland':
        PalDepth = SkyTEM['Model_Depths'][:, Npreq - NPL + 27]
        PalThick = SkyTEM['Model_Depths'][:, Npreq - NPL + 28] - SkyTEM['Model_Depths'][:, Npreq - NPL + 27]
    else:  # Fyn / Fyn-MST / Sjælland
    ...
    # f_doi(x) = min(doimean, x + 1000 * (PalThick < 10))
    estimated_DOIs = np.minimum(doimean, PalDepth + 1e3 * (PalThick < 10))
    DOI_NaN_Filter = np.isnan(DOIs)
        ...
        for i in range(ny):
            ...
            for j in range(nx):
                ...
                if np.any(filt):
                    ...
                    # NB: SkyTEM uses the minimum (not mean) model thickness here
    ...
    # var0 driven by model thickness; cells shallower than 5 m get a huge variance
